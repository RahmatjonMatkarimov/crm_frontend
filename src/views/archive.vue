...
<template>
    <div class="space-y-5">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold" :style="themeStore.isDark ? 'color:#e2e8f0' : 'color:#1a1f36'">{{ $t('Arxiv') }}</h1>
                <p class="text-sm mt-0.5" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#4a5568'">{{ $t('Arxivlangan mijozlar va ishchilar') }}</p>
            </div>
            <button v-if="search || dateFrom || dateTo" @click="clearFilters"
                ...
                :style="themeStore.isDark
                    ? 'color:#8892a4; background:#1e2d42; border:1px solid #1e2d42;'
        ...
        <!-- Tabs -->
        <div class="flex gap-1 p-1 rounded w-fit" :style="themeStore.isDark ? 'background:#1e2d42; border:1px solid #1e2d42;' : 'background:#eaecf0; border:1px solid #d8dde6;'">
            <button @click="switchTab('customers')"
                ...
                :style="activeTab === 'customers'
                    ...
                    : themeStore.isDark ? 'color:#8892a4;' : 'color:#4a5568;'">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                {{ $t('Mijozlar') }}
                <span class="px-2 py-0.5 rounded text-[10px] font-bold"
                    :style="activeTab === 'customers' ? 'background:rgba(255,255,255,0.2); color:#fff;' : themeStore.isDark ? 'background:#1e2d42; color:#8892a4;' : 'background:#d8dde6; color:#4a5568;'">
            ...
            <button @click="switchTab('users')"
                ...
                {{ $t('Ishchilar') }}
                <span class="px-2 py-0.5 rounded text-[10px] font-bold"
                    :style="activeTab === 'users' ? 'background:rgba(255,255,255,0.2); color:#fff;' : themeStore.isDark ? 'background:#1e2d42; color:#8892a4;' : 'background:#d8dde6; color:#4a5568;'">
        ...
        <div class="flex flex-col md:flex-row gap-3 flex-wrap">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <span class="text-xs font-medium shrink-0" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#4a5568'">{{ $t('Arxiv sanasi:') }}</span>
        ...
        <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 -translate-y-2" leave-active-class="transition-all duration-150" leave-to-class="opacity-0 -translate-y-2">
            <div v-if="activeTab === 'customers' && selectedC.length > 0"
                ...
                <span class="text-sm font-medium" :style="themeStore.isDark ? 'color:#8892a4' : 'color:#1a1f36'">{{ selectedC.length }} {{ $t('ta tanlandi') }}</span>
                <div class="flex gap-2">
                    <button @click="selectedC = []" class="px-4 py-1.5 rounded text-sm transition-colors" :style="themeStore.isDark ? 'color:#8892a4;' : 'color:#4a5568;'">{{ $t('Bekor qilish') }}</button>
        ...
        <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 -translate-y-2" leave-active-class="transition-all duration-150" leave-to-class="opacity-0 -translate-y-2">
            <div v-if="activeTab === 'users' && selectedU.length > 0"
                class="flex items-center justify-between px-5 py-3 rounded"
                :style="themeStore.isDark ? 'background:#161c2d; border:1px solid #1e2d42;' : 'background:#eef2f7; border:1px solid #d8dde6;'">
                <span class="text-sm font-medium" :style="themeStore.isDark ? 'color:#8892a4' : 'color:#1a1f36'">{{ selectedU.length }} {{ $t('ta tanlandi') }}</span>
                <div class="flex gap-2">
                    <button @click="selectedU = []" class="px-4 py-1.5 rounded text-sm transition-colors" :style="themeStore.isDark ? 'color:#8892a4;' : 'color:#4a5568;'">{{ $t('Bekor qilish') }}</button>
        ...
        <div v-if="activeTab === 'customers' && filteredCustomers.length > 0"
            ...
            <span class="text-xs font-medium" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#4a5568'">
        ...
        <div v-if="activeTab === 'customers'">
            ...
            <div v-else-if="filteredCustomers.length > 0" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                <div v-for="c in filteredCustomers" :key="c.id"
                    ...
                    :style="[
                        themeStore.isDark
                            ? 'background:#161c2d; border:1px solid ' + (selectedC.includes(c.id) ? '#b45309' : '#1e2d42') + ';'
                    ...
                    <div class="space-y-2 mb-4">
                        <div class="flex items-center gap-2">
                            <svg class="w-3.5 h-3.5 shrink-0" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#8892a4'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#8892a4'">
            ...
            <div v-else class="flex flex-col items-center justify-center py-20 gap-4">
                <div class="w-20 h-20 rounded flex items-center justify-center" :style="themeStore.isDark ? 'background:#1e2d42' : 'background:#eaecf0'">
                    <svg class="w-10 h-10" :style="themeStore.isDark ? 'color:#1e2d42' : 'color:#c1c9d6'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p class="text-sm" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#8892a4'">{{ $t('Arxivda mijoz topilmadi') }}</p>
            </div>
        </div>
        ...
        <div v-if="activeTab === 'users'">
            ...
            <div v-else-if="filteredUsers.length > 0" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                <div v-for="u in filteredUsers" :key="u.id"
                    ...
                    :style="[
                        themeStore.isDark
                            ? 'background:#161c2d; border:1px solid ' + (selectedU.includes(u.id) ? '#b45309' : '#1e2d42') + ';'
                    ...
                    <div class="space-y-2 mb-4">
                        <div v-if="u.phone" class="flex items-center gap-2">
                            ...
                            <span class="text-xs" :style="themeStore.isDark ? 'color:#8892a4' : 'color:#4a5568'">{{ u.phone }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="w-3.5 h-3.5 shrink-0" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#8892a4'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 3.75V16.5L12 14.25 7.5 16.5V3.75m9 0H18A2.25 2.25 0 0120.25 6v12A2.25 2.25 0 0118 20.25H6A2.25 2.25 0 013.75 18V6A2.25 2.25 0 016 3.75h1.5m9 0h-9" />
            ...
            <div v-else class="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p class="text-sm" :style="themeStore.isDark ? 'color:#4a5878' : 'color:#8892a4'">{{ $t('Arxivda ishchi topilmadi') }}</p>
            </div>
        </div>
    </div>
</template>
