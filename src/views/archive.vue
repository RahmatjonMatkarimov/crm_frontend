...
<template>
    <div class="space-y-5">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="gov-page-title">{{ $t('Arxiv') }}</h1>
                <p class="text-sm mt-0.5" style="color:var(--text-2);">{{ $t('Arxivlangan mijozlar va ishchilar') }}</p>
            </div>
            <button v-if="search || dateFrom || dateTo" @click="clearFilters" class="btn btn-ghost">
        ...
        <div class="flex gap-1.5">
            <button @click="switchTab('customers')" :class="['period-tab', 'flex items-center gap-2', activeTab === 'customers' ? 'period-tab-active' : '']">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                {{ $t('Mijozlar') }}
                <span class="px-2 py-0.5 rounded-lg text-[10px] font-bold" style="background:color-mix(in srgb, currentColor 15%, transparent);">
                    {{ customersStore.archived.length }}
                </span>
            </button>
            <button @click="switchTab('users')" :class="['period-tab', 'flex items-center gap-2', activeTab === 'users' ? 'period-tab-active' : '']">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex flex-col md:flex-row gap-3 flex-wrap">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <span class="text-xs font-medium shrink-0" style="color:var(--text-2);">{{ $t('Arxiv sanasi:') }}</span>
                <input v-model="dateFrom" type="date" class="form-input w-auto" />
                <span style="color:var(--text-2);">—</span>
                <input v-model="dateTo" type="date" class="form-input w-auto" />
            </div>
        </div>
        ...
        <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 -translate-y-2" leave-active-class="transition-all duration-150" leave-to-class="opacity-0 -translate-y-2">
            <div v-if="activeTab === 'customers' && selectedC.length > 0"
                ...
                <span class="text-sm font-medium" style="color:var(--text-1)">{{ selectedC.length }} {{ $t('ta tanlandi') }}</span>
        ...
        <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 -translate-y-2" leave-active-class="transition-all duration-150" leave-to-class="opacity-0 -translate-y-2">
            <div v-if="activeTab === 'users' && selectedU.length > 0"
                class="flex items-center justify-between px-5 py-3 rounded-lg"
                style="background:var(--primary-light); border:1px solid var(--border);">
                <span class="text-sm font-medium" style="color:var(--text-1)">{{ selectedU.length }} {{ $t('ta tanlandi') }}</span>
                <div class="flex gap-2">
        ...
        <div v-if="activeTab === 'customers'">
            ...
            <div v-else-if="filteredCustomers.length > 0" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                <div v-for="c in filteredCustomers" :key="c.id"
                    class="relative rounded-lg p-5 transition-all duration-200 group card"
                    :style="{ borderColor: selectedC.includes(c.id) ? 'var(--primary)' : 'var(--border)' }">

                    <!-- Archive badge -->
                    <div class="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
                        style="background:var(--warning-bg); border:1px solid var(--warning-border);">
                        <svg class="w-3 h-3" style="color:var(--warning);" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                        </svg>
                        <span class="text-[10px] font-bold" style="color:var(--warning);">{{ formatDate(c.archivedAt) }}</span>
                    </div>
            ...
            <div v-else class="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p class="text-sm" :style="'color:var(--text-2)'">{{ $t('Arxivda mijoz topilmadi') }}</p>
            </div>
        </div>
        ...
        <div v-if="activeTab === 'users'">
            ...
            <div v-else-if="filteredUsers.length > 0" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                <div v-for="u in filteredUsers" :key="u.id"
                    class="relative rounded-lg p-5 transition-all duration-200 card"
                    :style="{ borderColor: selectedU.includes(u.id) ? 'var(--primary)' : 'var(--border)' }">
                    ...
                    <div class="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
                        ...
                        <span class="text-[10px] font-bold" style="color:var(--warning);">{{ formatDate(u.archivedAt) }}</span>
                    </div>
                    ...
                    <div class="space-y-2 mb-4">
                        <div v-if="u.phone" class="flex items-center gap-2">
                            ...
                            <span class="text-xs" :style="'color:var(--text-2)'">{{ u.phone }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="w-3.5 h-3.5 shrink-0" :style="'color:var(--text-2)'" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 3.75V16.5L12 14.25 7.5 16.5V3.75m9 0H18A2.25 2.25 0 0120.25 6v12A2.25 2.25 0 0118 20.25H6A2.25 2.25 0 013.75 18V6A2.25 2.25 0 016 3.75h1.5m9 0h-9" />
            ...
            <div v-else class="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p class="text-sm" :style="'color:var(--text-2)'">{{ $t('Arxivda ishchi topilmadi') }}</p>
            </div>
        </div>
    </div>
</template>
